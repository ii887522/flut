...
impl<T> SparseSet<T> {
  ...
  pub fn add(&mut self, item: T) -> Id {
    ...
    let id = if let Some(free_id) = self.free_ids.pop() {
      self.sparse[free_id as usize] = AtomicU32::new(index as _);
      free_id
    } else {
      let id = self.sparse.len() as _;
      self.sparse.push(AtomicU32::new(index as _));
      id
    };
  ...
  pub fn remove(&mut self, id: Id) {
    ...
    if let Some(swapped_id) = self.ids.get_mut(dense_index as usize) {
...
impl<T: Send> SparseSet<T> {
  pub fn bulk_add(&mut self, items: Box<[T]>) -> Box<[Id]> {
    let count = items.len();
    ...
    let start_id = self.sparse.len() as u32;
    let start_index = start_index + ids.len();
    let new_ids_count = count - free_id_count;

    self.sparse.par_extend(
      (start_index..start_index + new_ids_count)
        .into_par_iter()
        .with_min_len(MIN_SEQ_LEN)
    ...
    ids.par_extend(
      (start_id..start_id + new_ids_count as u32)
        .into_par_iter()
        .with_min_len(MIN_SEQ_LEN),
    );
  ...
  pub fn bulk_update(&mut self, updates: Box<[(&Id, T)]>) {
    updates
      .into_par_iter()
      .with_min_len(MIN_SEQ_LEN)
      .for_each(|(id, item)| unsafe {
        *self.dense[self.sparse[id.0 as usize].load(Ordering::Relaxed) as usize]
          .0
          .get() = item;
      });
  }
...
impl<T: Clone + Send> SparseSet<T> {
  pub fn bulk_remove(&mut self, ids: &[Id]) {
    ...
    dense_indices_to_replace
      ...
      .for_each(|(&index_to_replace, index_to_keep)| unsafe {
        *self.dense[index_to_replace as usize].0.get() = self.dense[index_to_keep].clone();
